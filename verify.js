...
window.verifyKey = async function () {
  ...
  // step 4: 空密钥提示Add commentMore actions
  if (!key) {
  ...
  try {
    ...
    // step 6: 无此密钥Add commentMore actions
    if (!keySnap.exists()) {
    ...
    if (isUsed && boundDevice && boundDevice !== deviceId) {
      result.textContent = "此密钥已被其他设备绑定";Add commentMore actions
      return;
    }
    ...
    // step 9: 检查密钥是否未生效Add commentMore actions
